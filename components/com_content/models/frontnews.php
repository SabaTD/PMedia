...
//		$item = $db->loadObjectList();
	...
	function getData( $sec_cat_id, $main_news, $comment, $conference, $inneradvert, $big_advert, $main_advert, $right_advert, $gallery, $galleryitem, $i, $newsmode, $othernews )
	{
		$sec_cat = explode( '.', $sec_cat_id );

		$data = $this->getArticle( $sec_cat, $main_news, $newsmode, $othernews );
		$data['fitem'] = $i;
	...
	function getArticle( $sec_cat, $main_news, $newsmode, $othernews )
	{
		global $mainframe;
//		$db = & JFactory::getDBO();
		...
		SELECT * FROM `#__categories` where id = ' . $sec_cat[1];
			...
			{
				$query = '#FrontNews 
		(SELECT `a`.`id` , `a`.`title` , `a`.`alias` , `a`.`introtext` , `a`.`fulltext` , `a`.`publish_up` , `a`.`sectionid` , `c`.`id` as `catslug`, `s`.`title` as `sec_title` , `c`.`title` as `cat_title`, `a`.`attribs`, 1 as `sortkey` '
								. ' FROM `#__content` AS `a`'
								. ' LEFT JOIN `#__categories` AS `c` ON `c`.`id` = `a`.`catid`'
								. ' LEFT JOIN `#__sections` AS `s` ON `c`.`section` = `s`.`id`'
								. ' WHERE `a`.`id` = ' . $main_news
...
//								. ' OR `a`.`publish_down` >= ' . $this->_db->Quote( $now ) . ' )'
								. ')'
								. ' UNION ALL '
								. ' (SELECT `a`.`id` , `a`.`title` , `a`.`alias` , `a`.`introtext` , `a`.`fulltext` , `a`.`publish_up` , `a`.`sectionid` , `c`.`id` as `catslug`, `s`.`title` as `sec_title` , `c`.`title` as `cat_title`, `a`.`attribs`,  2 as `sortkey`'
								. ' FROM `#__content` AS `a`'
								. ' LEFT JOIN `#__categories` AS `c` ON `c`.`id` = `a`.`catid`'
								//	. 'RIGHT JOIN `#__newsconfig` AS `nc` ON (`nc`.`exclusive` <> `a`.`catid` or `nc`.`conference` <> `a`.`catid`)'
								. ' LEFT JOIN `#__sections` AS `s` ON `c`.`section` = `s`.`id`'
								. ' WHERE `a`.`state` =1 '
								//	. ' and `nc`.`exclusive` <>`a`.`catid`and `nc`.`conference` <> `a`.`catid` '
								. ' AND (`a`.`catid` = '
//			.' AND `a`.`catid` = "' 
								. $sec_cat[1]
			...
			else
			{
				...
				$otherA = array( $main_news );

				foreach ( $othernews as $value )
				{
					$value = trim( $value );
					if ( empty( $value ) )
					{
				...
				$o = implode( ',', $otherA );
				$query = '#FrontNews Manual Mode Query 1' . "\n"
...
//		$rows = $db->loadObjectList();
		...
		//	$isFirst = $i == 0 and ( $row->id == $main_news);
			$isFirst = ( $row->id == $main_news) ? true : false;
	...
	function getBanner( $params )
	{
		...
		$BannersF = str_replace( '&', '&amp;', str_replace( '&amp;', '&', $Banners ) );
		return $BannersF;
