 ...
 * @since 1.5
 */
class ContentModelTags extends JModel
{
	/**
	 * Category id
	 *
	...
	function getTotal( $state = 1 )
	{
		...
		if ( empty( $this->_total ) )
		{
			...
			$query = $this->_buildTotalQuery( $state );
			$this->_db->setQuery( $query );
			$k = $this->_db->loadResult();
	...
	function _buildTotalQuery( $state = 1 )
	{
		global $mainframe;
		...
		$leyout = JRequest::getVar( 'layout', '' );

		$orderby = $this->_buildContentOrderBy( $state );
		if ( $leyout == 'blogmenu' )
		{
			$orderby = ' ORDER BY `cc`.`ordering`,`a`.`catid`';
		}

		$query = '#Tag total Select' . "\n"
						. 'SELECT count(*) ' .
						' FROM #__content AS a' .
						' INNER JOIN #__categories AS cc ON cc.id = a.catid' .
						' LEFT JOIN #__sections AS s ON s.id = a.sectionid' .
						' LEFT JOIN #__users AS u ON u.id = a.created_by' .
						' LEFT JOIN #__content_tags_rel AS tr ON tr.item_id = a.id' .
						' LEFT JOIN #__content_tags AS tagr ON tagr.id = tr.tag_id' .
						$where
						. $orderby
		;
	...
	function _loadData( $state = 1 )
	{
		...
		if ( empty( $this->_data[$state] ) )
		{
			...
			{
				JError::raiseError( 500, $this->_db->getError() );
				return false;
			}
	...
	function _buildQuery( $state = 1 )
	{
		...
		$query = '#Content Tags ' . PHP_EOL
						. 'SELECT a.id, a.title,  tagr.tag AS tag_title, a.alias, a.title_alias, a.introtext, a.fulltext, a.sectionid, a.state, a.catid, a.created, a.created_by, a.created_by_alias, a.modified, a.modified_by,' .
	...
	function _buildContentWhere( $state = 1 )
	{
		...
		if ( $noauth )
		{
			$where = ' WHERE a.state<100 AND a.access <= ' . (int) $aid;
		}
		else
		{
		...
		$tag_id = JRequest::getInt( 'tags', 0 );
		if ( !empty( $tag_id ) )
		{
			//$tag_id = explode( '-', $tag_id );
			$where .= ' AND tr.tag_id = ' . $tag_id;
		}
