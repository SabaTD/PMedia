...
class ContentViewCategory extends ContentView
{
	function display( $tpl = null )
	{
		...
		if ( $layout == 'nblog' )
		{
			require_once (JPATH_COMPONENT . DS . 'models' . DS . 'rubric.php');
			$model = new contentModelrubric();
			...
			if ( !empty( $rub->id ) )
			{
				...
				$data = $model->getData( $rub->section . '.' . $rub->category, $rub->main_news, $rub->comment, $rub->big_advert, $rub->main_advert, $rub->number, $rub->gallery, $rub->galleryitem, $rub->newsmode, $rub->othernews );
		...
		else
		{
			...
			$params->def( 'num_intro_articles', 10 );
	...
	function Redirect()
	{
		...
		JController::setMessage( 'მოთხოვნილი გვერდი არ არსებობს!' );
