...
class JHTMLIcon
{
	...
	function email( $article, $params, $access, $attribs = array() )
	{
		...
		$url = 'index.php?option=com_mailto&tmpl=popup&link=' . base64_encode( $link );

		$status = 'width=400,height=350,menubar=yes,resizable=yes';

		if ( $params->get( 'show_icons' ) )
		{
			$text = JHTML::_( 'image.site', 'emailButton.png', JPHOTO_DOMAIN . 'images/M_images/', NULL, NULL, JText::_( 'Email' ) );
		}
		else
		{
			$text = '&nbsp;' . JText::_( 'Email' );
	...
	{
		$url = 'index.php?view=article';
		$url .= isset( $article->catslug ) ? '&catid=' . $article->catslug : '';
		$url .= '&id=' . $article->slug . '&tmpl=popup&print=1&layout=default&page=' . JRequest::getVar( 'limitstart', 0 );
		...
		if ( $params->get( 'show_icons' ) )
		{
			$text = JHTML::_( 'image.site', 'printButton.png', JPHOTO_DOMAIN . 'images/M_images/', NULL, NULL, JText::_( 'Print' ) );
		}
		else
		{
